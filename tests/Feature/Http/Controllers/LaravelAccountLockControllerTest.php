...
class LaravelAccountLockControllerTest extends TestCase
{
    /** @test */
    public function it_locks_account_when_we_reach_the_url()
    {
        $user = UserFactory::new()->create();
        $url = (new LaravelAccountLock())->generateLockUrl($user->getTable(), $user->getKey(), 60);
    ...
    {
        $user = UserFactory::new()->create();
        $malformedUrl = (new LaravelAccountLock())->generateLockUrl($user->getTable(), $user->getKey(), 60) . 'aaa';
    ...
    {
        $user = UserFactory::new()->create();
        $expiredUrl = (new LaravelAccountLock())->generateLockUrl($user->getTable(), $user->getKey(), -60);
